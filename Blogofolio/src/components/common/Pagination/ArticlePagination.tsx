...
import {InnerPaginationProps} from "./ModalPagination";

import styles from "./Pagination.module.css";

interface ArticlePaginationProps extends InnerPaginationProps{
    nextTitle?: string | null ,
    prevTitle?: string | null,
}

const ArticlePagination:FC<ArticlePaginationProps > = ({nextTitle, prevTitle,onClickNext, onClickPrev, disabledPrev, disabledNext}) => {
    const {isDarkTheme} = useTheme();

    return (
        <div className={styles.paginationArticle}>
            <button className={styles.button} disabled={disabledPrev} onClick={onClickPrev}>
                ...
                <div className={styles.blockLeft}>
                    ...
                    {!!prevTitle &&  <p className={styles.title}>{prevTitle}</p>}
                </div>
            </button>
            <button className={styles.button} disabled={disabledNext} onClick={onClickNext}>
                <div className={styles.blockRight}>
                    <span className={`${isDarkTheme ? styles.dark : ""} ${styles.span}`}>Next</span>
                    {!!nextTitle &&  <p className={styles.title}>{nextTitle}</p>}
                </div>
                <svg className={`${isDarkTheme ? styles.dark : ""} ${styles.svg}`} width="18" height="15" viewBox="0 0 18 15" xmlns="http://www.w3.org/2000/svg">
