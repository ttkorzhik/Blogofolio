...
const Pagination:FC<PaginationProps> = ({items = []}) => {
    ...
    return (
        ...
        <div className={`${styles.pagination}`}>
            <button  data-route={"prev"} className={styles.button} onClick={handleChangePage} disabled={!previous}>
                ...
                <div className={styles.pages}>
                    {paginationItems.map((page, index) => {
                        if ((page + 1) !== paginationItems[index + 1] && page !== paginationItems.at(-1)) {
                            return (
                                <div key={index} className={styles.withPoints}>
                                    <button
                                            data-id={String(page - 1)}
                                            className={`${isDarkTheme ? styles.dark : ""} ${activePage === page ? styles.active : styles.page}`}
                                            onClick={handleSetActivePage}>
                                        {page}
                                    </button>
                                    <p className={`${isDarkTheme ? styles.dark : ""} ${styles.points}`}> ... </p>
                                </div>
                            )
                        } else {
                            return (
            ...
            <button data-route={"next"} className={styles.button} onClick={handleChangePage} disabled={!next}>
